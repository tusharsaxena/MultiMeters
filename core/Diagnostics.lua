...
-- to one open issue each moved out to core/Diagnostics_DeathRecap.lua (#1),
-- core/Diagnostics_Identity.lua (#22) and core/Diagnostics_Feign.lua (#25) —
-- each of which can now be deleted with its issue. They load immediately AFTER
...
-- ---------------------------------------------------------------------------
-- The frame, published for the sibling probe files
-- ---------------------------------------------------------------------------
--
-- The three peeled probes print through the SAME helpers rather than growing a
-- second definition of "safe to print" — which is the whole reason `safe` defers
-- to core/CoreSetup.lua above rather than answering for itself. `out` is
-- published as itself, so a sibling's line follows whatever route `emit`
-- currently names without the sibling having to know there is a route.
Diagnostics.out    = out
...
---
--- `emit` stays a file-local and the siblings set it through here rather than
--- keeping a copy, so there is still exactly one sink and one place that clears
--- it. A sibling holding its own would route half a report into the console and
--- the other half into chat, which is the failure this whole file exists to stop
--- somebody from having to diagnose from a screenshot.
---
--- @param fn function|nil
function Diagnostics.SetEmit(fn)
    emit = fn
end
