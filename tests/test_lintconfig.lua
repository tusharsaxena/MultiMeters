...
-- and the exit pass found it. It is the more instructive of the two shapes the collection had,
-- because every entry was already narrowed to a VARIABLE and it still cost the repository
-- thirty-two live defects: `211/addonName` at the top level does not mean "the bootstrap header is
-- allowed to be unread", it means "no file in this addon may ever report an unused `addonName`",
-- and thirty-two of them were unread. Removing the three lines reported eighty-four findings.
-- Thirty-two were fixed at source under `M4c-06`; the fifty-two that remain are receivers a calling
-- convention forces, and each sits in a per-file stanza that says which convention. A fourth entry
-- would have been just as invisible: `212/event` named a warning this repository does not produce
-- at all, and had been sitting in the config being obeyed by nothing.
--
-- The measurement that says the difference is real: an unread `local addonName, NS = ...` header
-- and an unused `self` planted together in core/State.lua — a file with no stanza — report as
-- `:24:7 (W211)` and `:155:15 (W212)` under the config this gate guards, and report zero warnings
-- under the blanket.
--
...
-- existing. Same bargain tests/test_docmap.lua and tests/_kit/test_eol.lua strike here.
...
test("lintconfig: .luacheckrc sets no top-level ignore", function()
    ...
    if ignore ~= nil then
        ...
        fail(".luacheckrc sets a top-level `ignore` of { " .. table.concat(shown, ", ") .. " }. "
            ...
            .. "`M4-11`). Narrowing the entry to a variable is not enough on its own — the list "
            .. "this replaced was already spelled `212/self`, and it still hid thirty-two unread "
            .. "`addonName` headers. Move each code into a `files[...]` stanza naming the file that "
            .. "earns it, or put a `-- luacheck: ignore <code>` beside the single line that needs "
            .. "it", 2)
    end
end)
...
test("lintconfig: no source file carries a bare inline luacheck ignore", function()
    ...
    for _, path in ipairs(trackedLua()) do
        ...
        for line in fh:lines() do
            ...
            -- is the form this rule steers towards. This repository has no user of either as of
            -- `M4c-06`: settings/ColumnBlocks.lua's `211/addonName` directive was hiding a real
            -- unread header and went with the other thirty-one, and core/CoreSetup.lua's was
            -- sitting over a header that does read the name, so it was silencing nothing.
