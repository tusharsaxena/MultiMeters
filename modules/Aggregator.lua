...
-- ---------------------------------------------------------------------------
-- Ordering and the row cap — peeled to modules/Aggregator_Order.lua
-- ---------------------------------------------------------------------------
--
-- The orderings (`reverseRows`, `orderByProvider`, `orderByValue`,
-- `orderByName`, `orderByRoster`) and the cap (`Aggregator.ApplyRowLimit`,
-- `Aggregator.SelfPinIndex`) sit in modules/Aggregator_Order.lua, which loads
-- after this file. They left under layout-§1 in the 2026-09-26 automated-tests
-- sweep (ATS-12), and they are the one part of stages 4 and 5 that could leave
-- without cutting the pipeline: each takes a row array and returns, reads no
-- pass table and no file-local of this one but `UNRANKED`. The LADDER that
-- picks among them, `applySortMode`, is a stage and reads the pass, so it stays
-- below where the header's narration puts it.
--
-- This table is the seam. `UNRANKED` travels out on it; the sibling hangs the
-- orderings back on it at load, and this file calls them through it. Every
-- caller is inside a Build, which cannot run until the whole TOC has loaded.
local Order = { UNRANKED = UNRANKED }
Aggregator._order = Order
...
local function applySortMode(pass)
    ...
    if mode == "roster" then
        Order.orderByRoster(rows)
        return mode
    end

    if mode == "name" then
        if Order.orderByName(rows, pass.sortAscending) then return mode end
        Order.orderByProvider(rows, pass.sortAscending)
        return "provider"
    end
    ...
    if Order.orderByValue(rows, pass.sortColumn, pass.sortAscending) then return mode end
...
function Aggregator.Build(a, b, c)
    ...
    if pass.identityMode then
        ...
        -- in modules/Aggregator_Order.lua), so it is legal on secrets — and it
        -- puts the rows the sort column never named first, which is the same
        -- answer `value` mode gives out of combat for a missing cell.
        Order.orderByProvider(pass.rows, pass.sortAscending)
