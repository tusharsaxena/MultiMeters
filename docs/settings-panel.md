...
console) are `sessionOnly` rows the `MasterControls` composer emits, each dressed with its own
`get`/`set` — so neither can accidentally become a stored value — and its reset buttons are drawn by the hook `H.MasterControls` returns rather
