...
the always-shown scrollbar patch belong to `LibKa0s-Options-1.0` (`libs/LibKa0s/Options*.lua`, v1.35.0
...
|---|---|---|---|---|---|---|---|
| 1 | General | `general` | 22 (18 visible + 4 `hidden`) | 2 — **Master controls**, **Statistic colors** | yes | no | **Master controls** — `options-ui-§15`'s canonical set, first on this page in every Ka0s addon and [composed rather than written out](#the-composed-blocks): Enable Multi Meters, General visibility, Master scale, Master alpha, Lock frame, Debug console, Test mode — then this addon's own three (the minimap button, then **Merge pets** and **Refresh interval**, both addon-wide since schemaVersion 5), closed by the **Reset position** / **Reset all settings** button pair the composer hands back. Master scale and Master alpha are ADDON-WIDE multipliers, not the per-window scale and opacity on the Frame page, and **Lock frame** is a session-only view over every window's own lock rather than a second lock — see [The Master controls tab](#the-master-controls-tab). **Statistic colors** — one swatch per entry of `Constants.STAT_COLORS`, [generated rather than written out](#the-statistic-palette), read back through `NS.StatColor`, with a note under the grid saying where those colours are actually worn (drawn through the same `afterGroup` hook, keyed to this tab). A fourth schema group, **Export**, holds the export modal's four remembered choices — all four `hidden`, so the group is real for `/mm list` and the schema-vs-defaults validator and never appears as a tab: this is the one *section that is not a tab*, and a wholly hidden group is not a strip-less page. General is not a window page, so it draws no banner. |
...
**General is not one of them any more.** Its three session-only toggles (Test mode, the debug
console and Lock frame) are `sessionOnly` rows the `MasterControls` composer emits, each dressed with its own
`get`/`set` — so none can accidentally become a stored value — and its reset buttons are drawn by the hook `H.MasterControls` returns rather
...
- **Lock frame (`master.locked`) is a view over every window's own lock, not a second lock.** That
  is a ratified deviation from `options-ui-§15`'s per-instance clause
  ([ARCHITECTURE.md → Documented deviations](ARCHITECTURE.md#documented-deviations)). The box reads
  ticked only while every window's **Lock window** is on, and ticking or unticking it locks or
  unlocks every window through `WindowManager:SetLocked`, the switch `/mm lock on` and
  `/mm lock off` use. It is `sessionOnly` because its value is derived: what persists is each
  window's `frame.locked`. A window's own padlock (in its header, or Frame → Lock window) still
  locks that window alone, and the box then reads unticked. It used to be a stored lock ORed over
  every window's own, and after `/mm lock` the box could neither unlock a window nor visibly lock
  one.
...
because a `group` is not a stored path.
