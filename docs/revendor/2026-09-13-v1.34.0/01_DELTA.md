...
`MasterControls`, the composer whose Reset-all tooltip moved, on a table attached at `:484` (line numbers
at the re-vendor commit) with no descriptor, until B1 gave that call a compose descriptor
(02_CANDIDATES B1). `Slash` is looked up at
