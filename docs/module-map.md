...
| File | Owns | Publishes | Consumes |
|---|---|---|---|
| `Compat.lua` | All 29 cross-patch shims — the TOC-manifest reader is no longer among them; it moved to `EnvSetup.lua`. Never inspects a meter value; reading a field off a session table and passing it on is not inspection. The four spell and spec readers are bound to `LibKa0s-Compat-1.0`, and each answers `nil` without it | `NS.Compat` | `_G`, `LibKa0s-Compat-1.0` |
