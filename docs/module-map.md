...
└── settings/
    ...
    ├── Schema.lua      — NS.Schema: the 168-row array itself, and nothing else. The
...
| `Schema.lua` | **The array, and nothing else.** 168 rows across 8 page keys, each one the single source of truth for a setting's widget, its CLI parser and its default. This is the file that grows, and a new setting is still one row in it | `NS.Schema` | every member of `NS.SchemaCompose`, resolved **at file scope** as the array is declared — which is what puts `settings/Schema_Compose.lua` ahead of it in the TOC |
...
Schema rows total 168 across 8 page keys — windows 1, frame 26, header 36, bars 29, tooltip 30,
