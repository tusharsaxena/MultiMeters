...
Fifty-seven non-vendored source files: 1 locale, 18 `core/`, 1 `defaults/`, 22 `modules/`,
15 `settings/`.
...
| `Slash.lua` | `NS.COMMANDS`, the five schema adapters pointed at the seam above, the six host verbs, the library-absent stub, and the disabled gate — one wrap over the verb table that refuses any verb not named in `ALWAYS_LIVE` while `enabled` is false (`slash-commands-§2`) | `NS.Slash` — `Register`, `OnSlash`, `PrintHelp`, `HelpRows`, `LandingRows`, `Version` | LibKa0s-Slash-1.0, `NS.WindowManager`, `NS.DebugLog`, `NS.Perf`, `NS.Export`, the schema seam |
