...
behavior into a peeled file is reading the block that used to sit under a banner in its parent.
...
│                         C_SpecializationInfo, all eight C_DamageMeter reads,
│                         C_StringUtil.CreateNumericRuleFormatter and the chat
│                         sender C_ChatInfo.SendChatMessage. 30 shims, no logic
...
│                         private-bus-target factory over LibKa0s-Bus-1.0's
│                         stand-down record (NS.BusStandDown / NS.BusStandUp)
...
│   ├── LifecycleSetup.lua — LibKa0s-Lifecycle-1.0 seam: THE ONE LATCH and THE ONE
...
│   │                     descriptor and nothing else: the folder name, the logo, how
│   │                     the panel opens and the options menu's four verb-backed pairs.
...
│   ├── Diagnostics.lua — `/mm diagnostics`: the sectioned report a player pastes into
...
│   └── Diagnostics_Runtime.lua
│                       — the addon-state sections that open `/mm diagnostics`:
│                         state, settings, per-window settings, windows, sessions,
│                         the aggregator's last pass, roster and caches. Read-only
├── defaults/
...
└── settings/
    ...
    ├── Slash.lua       — LibKa0s-Slash-1.0 seam: NS.COMMANDS (19 verbs), the five
    ...
    │                     instance, the panel registry, the reset-all veto, and the
    │                     one row no reset may write (the minimap button)
    ├── ColumnBlocks.lua — the ROW CONTENTS of the Columns entry's reorderable
    │                     list. The gesture and the chrome are LibKa0s-Widgets-1.0's
    │                     ReorderList; loads BEFORE Columns.lua
    └── General · Windows · Profiles, and Frame · Header · Bars ·
        Tooltip · Visibility · Columns
                          — the 3 panel pages (General, Windows, Profiles) and the
                          Windows page's 7 entries. General is FIRST; the six
                          files after Windows.lua register entries on its rail,
                          under the band (H.WindowBanner) that picks the window
```
...
| File | Owns | Publishes | Consumes |
|---|---|---|---|
| `Compat.lua` | All 30 cross-patch shims — the TOC-manifest reader is no longer among them; it moved to `EnvSetup.lua`. Never inspects a meter value; reading a field off a session table and passing it on is not inspection. The four spell and spec readers are bound to `LibKa0s-Compat-1.0`, and each answers `nil` without it | `NS.Compat` | `_G`, `LibKa0s-Compat-1.0` |
| `Constants.lua` | The stat catalog, the **shipped** per-stat palette (`STAT_COLORS`, the seed for the `statColors.*` setting and the fallback every `NS.StatColor` call degrades to, plus the `STAT_DIM` factor), the two stat lookups — `STAT_BY_KEY` ("may this be a column") and `READABLE_STAT_BY_KEY` ("may this be read", the catalog plus `OFF_CATALOG_STATS`), enum resolutions, the `MSG` catalog (strict through `LibKa0s-Bus-1.0`'s `Catalog`), timing and pool bounds, the monospace font path (resolved from the LibKa0s payload, falling back to the client font) and its LSM key | `NS.Constants`, `NS.Const` | `_G.Enum`, `NS.MediaFont`, `LibKa0s-Bus-1.0` (`Catalog`) |
...
| `Namespace.lua` | Addon identity, the collection's one class-color reader (four surfaces can wear one — the bars and cell text in `Row.lua`, both header strips in `Window.lua`, the tooltip), the one **statistic**-palette reader beside it (`NS.StatColor`, which reads the `statColors.*` setting and falls back to `Constants.STAT_COLORS`) and the bus-target factory, whose record is `LibKa0s-Bus-1.0`'s, with the untracked-target stub when the library is absent. No side effects at all: building the record is bookkeeping, and nothing registers | `NS.PREFIX`, `NS.GRAY`, `NS.name`, `NS.version`, `NS.FALLBACK_VERSION`, `NS.ClassRGB`, `NS.PlayerClassRGB`, `NS.StatColor`, `NS.NewBusTarget`, `NS.BusStandDown`, `NS.BusStandUp`, `NS.busRecord`, `NS.BusLib` | `NS.Meta`, `_G.RAID_CLASS_COLORS`, `_G.UnitClass`, `LibKa0s-Bus-1.0`, `NS.IsStoodDown` (late-bound) |
...
| `Secrets.lua` | Restriction state, per-value and per-table inspection, and the two bounded walks. Correct when the whole secrets system is absent. `IsSecret` / `CanAccess` / `IsSafeKey` are `LibKa0s-Compat-1.0`'s, with a guard arm that re-implements each body when the library is absent | `NS.Secrets` | `_G.C_RestrictedActions`, `_G.issecretvalue` and friends, `LibKa0s-Compat-1.0` |
...
| `CoreSetup.lua` | The LibKa0s-Core seam and the shared "library missing" cause clause. `NS.MakeCloseButton` is the one member WRAPPED rather than handed over: it supplies the addon folder name the library needs to build a texture path for its own close icon | `NS.Util.print` / `NS.Print`, `NS.Format` (printer, later rebound), `NS.IsConcatSafe`, `NS.SafeToString`, `NS.RGBA`, `NS.SKIN`, `NS.ApplySkin`, `NS.MakeCloseButton`, `NS.SafeRegisterEvent` / `NS.SafeRegisterUnitEvent` / `NS.SafeRegisterEvents`, `NS.LIBKA0S_MISSING` | `NS.PREFIX` |
| `LifecycleSetup.lua` | The ONE `LibKa0s-Lifecycle-1.0` latch and the addon's ONE teardown: `standDown` / `standUp`, the `disabled` hold taken from the stored `enabled` path, and the show ladder's answer. **Loads before `PerfSetup.lua`** — the perf descriptor requires `lifecycle` and raises without it | `NS.lifecycle`, `NS.StandDown`, `NS.StandUp`, `NS.SyncEnabledHold`, `NS.IsStoodDown`, `NS.IsDisabled` | `NS.GetSetting`, and `Provider` / `WindowManager` / `Visibility` / `Export` at call time |
| `PerfSetup.lua` | The perf descriptor: bucket list, the latch, log routing. **No `suspend` / `resume`** — from Perf minor 12 nothing reads them and the teardown is the latch's | `NS.Perf` | `NS.Version`, `NS.lifecycle` |
...
| `LauncherSetup.lua` | The LibKa0s-Launcher seam: the descriptor for the ONE LibDataBroker object LibDBIcon and any broker display both draw from (`launcher-§1`). Holds what is genuinely this addon's — the **folder** name (LibDBIcon keys the button's saved position by it), the logo, `version` from the TOC for the status tooltip's title (Launcher minor 3; the library draws the tooltip and this file passes no `onTooltipShow`), and the options menu's four accessor-and-toggle pairs (Launcher minor 4, `launcher-§2` at v2.67.0): `isEnabled` = `not NS.IsDisabled()` with `setEnabled` → the `enable`/`disable` verb, `isLocked` = `WindowManager:IsLocked` with `toggleLock` → `lock`, `isTestMode` = `NS.State.testMode` with `toggleTestMode` → `test`, `isWindowShown` = `WindowManager:AnyShown` with `toggleWindow` → `toggle`. Every toggle is the verb's own handler, looked up in `NS.COMMANDS` at click time. Left-click opens the panel (`openSettings`), in either state. The `minimap` field is a **closure**, never the table: `db.global.minimap` does not exist when core/ loads. The inversion is NOT here — it is the write seam's carve-out | `NS.Launcher` — `Register`, `IsRegistered`, `Object`, `IsShown`, `SetShown` | `LibKa0s-Launcher-1.0`, `NS.Constants.LOGO_128` (at file scope), `NS.db.global.minimap`, `NS.WindowManager`, `NS.OpenOptionsPanel`, `NS.COMMANDS`, `NS.IsDisabled` (all at call time) |
| `MultiMeters.lua` | AceAddon promotion, the printer reclaim, all 22 game-event registrations (the `EVENTS` array, each through `NS.SafeRegisterEvent`, refusals in `NS.State.rejectedEvents`), the fan-out onto the bus, and `NS.ShouldShow` | `NS.addon`, `NS.ShouldShow`, `NS:OnInitialize` / `OnEnable` | `NS.Constants.MSG`, `NS.State`, `NS.Secrets`, `NS.Launcher`, `NS.CreateOptionsPanel`, `NS.Slash` |
...
| `Diagnostics.lua` | The section list of the `/mm diagnostics` report and its client probes: atlas probes, the formatter ladder, visibility, header, name column, cells, tooltip font and width, the Targets cross-reference, the provider-order probe, and the rejected events, which close the report. The addon-state sections that open it live in `Diagnostics_Runtime.lua` and are reached through `NS.Diagnostics.Runtime` when they run. LibKa0s-DebugLog-1.0 runs them (`RunDiagnostics`, through the descriptor's `diagnostics` field in `core/DebugLogSetup.lua`), each under its own pcall, and nothing here inspects a meter value. It is also the FRAME the three probe files hang off — the print helpers are defined once, here | `NS.Diagnostics` (`Sections`, `SetEmit`) and, for the three files below, the shared helpers `out` / `safe` / `shown` / `probe` / `SECRET` | `NS.DebugLog`, `NS.Print`, `NS.Provider`, `NS.Constants.STATS`, `NS.Database`, `NS.Secrets`, `NS.WindowManager` |
| `Diagnostics_DeathRecap.lua` | `/mm debug recap` — issue #1's question, asked in one place: can a Death Recap window be built at all, and what does the recap API actually hand back. Self-contained, so it can be deleted with its issue; it also rides along in `/mm diagnostics`, which is why its section function goes back onto the shared table | `Diagnostics.ReportDeathRecap`, `Diagnostics.reportDeathRecap` (the section the frame calls) | `NS.Diagnostics` and its print helpers, **at file scope** — the reason its TOC line must follow `core/Diagnostics.lua` |
| `Diagnostics_Identity.lua` | `/mm debug identity` — the issue #22 identity-correlation measurement: which fields could key a row while `sourceGUID` is secret, and what each one costs when it is absent. **Not** part of `/mm diagnostics`; nothing in the frame reaches into it | `Diagnostics.ReportIdentity` | `NS.Diagnostics` and its `out`, **at file scope**. Prints through the frame's `out` rather than a second printer, so a line from here takes the route the report set |
| `Diagnostics_Runtime.lua` | The addon-state sections of `/mm diagnostics` (DX-MM), written straight to the library's section writer: the latch and its holds, both schema stamps, the profiles, the restriction (mirror, authority, raw state), the session flags and the availability memo; the profile's rows that differ from their defaults plus `enabled`, `master.visibility` and `data.mergePets` always; each window diffed against `NS.DefaultWindow` for its id and name, position from config; the window inventory with the ladder's live and remembered answers; the sessions the client holds and what each window reads; the aggregator's last render pass; the roster and cache counts. Read-only: no reset, no invalidation, no roster build, no geometry off a frame. While stood down, sessions and aggregator say so | `Diagnostics.Runtime` (`state`, `settings`, `windowSettings`, `windows`, `sessions`, `aggregator`, `rosterAndCaches`) | `NS.Diagnostics` **at file scope**; `NS.Schema`, `NS.GetSetting`, `NS.DefaultWindow`, `NS.Database`, `NS.lifecycle`, `NS.Secrets`, `NS.Provider`, `NS.Aggregator`, `NS.Visibility`, `NS.WindowManager`, `NS.State` at call time |
...
| `Slash.lua` | `NS.COMMANDS`, the five schema adapters pointed at the seam above, the six host verbs, the library-absent stub, and the descriptor's `isEnabled` / `brandName` — the LIBRARY's disabled gate, with **no `liveVerbs`**, so the live set is the standard's thirteen reserved verbs and only this addon's own feature verbs refuse (`slash-commands-§2`, `§7`) | `NS.Slash` — `Register`, `OnSlash`, `PrintHelp`, `HelpRows`, `LandingRows`, `DisabledLine`, `Version` | LibKa0s-Slash-1.0, `NS.WindowManager`, `NS.DebugLog`, `NS.Perf`, `NS.Export`, the schema seam |
| `OptionsSetup.lua` | The options descriptor, the page registry, the reset-all veto (`page == "profiles"`), the one row no reset in the panel may write (`global.minimap.shown`, exempted at `applyDefault` — `launcher-§3`), the Windows page's entry registry, `RenderWindowPage`, `SelectSection`, `OpenOptionsPage` (MultiMeters#55), and the library-absent stub | `NS.Helpers` (the library instance itself), `NS.CreateOptionsPanel`, `NS.OpenOptionsPanel`, `NS.RefreshOptionsPanel`, `NS.RegisterWindowSection`, `NS.WindowSection`, `Helpers.RenderWindowPage`, `Helpers.RestoreActiveSection` | LibKa0s-Options-1.0, `NS.Schema`, `NS.Slash:LandingRows` |
| `Windows.lua` | The window picker — `H.WindowBanner`, **the only writer of `NS.State.activeWindowId`**, decorated onto the library instance and drawn once, as the Windows page's band above its rail — and the Windows page's General entry (MultiMeters#55): the five registry buttons plus the Copy settings from block with its group filter, on one bespoke General tab | a page registration and a Windows-page entry | `NS.WindowManager`, `NS.State.SetActiveWindow`, `NS.RefreshOptionsPanel` |
| `Frame.lua` | Windows-page entry: registers `frame` (26 rows across four tabs: General, Size and position, Background and border, Row). Pure schema — the header controls moved to Header and "Reset position" to General | a Windows-page entry | `NS.Helpers` |
| `Header.lua` | Windows-page entry: registers `header` (36 rows across four tabs): the title bar's own text, every toggle for the icon strip it carries (Controls) and how those controls are drawn (Button style). The column-header strip moved to the Columns entry it labels. Rows are stored at `window.frame.*` / `window.header.*`. Pure schema | a Windows-page entry | `NS.Helpers` |
| `Bars.lua` | Windows-page entry: registers `bars` (29 rows across six tabs: Bar, Background, Border, Text content, Text style, Icons): the bar, its background, its border, the cell's two text slots and the row icon. The Text and Icons pages folded in here; their PATHS did not move with them. Pure schema | a Windows-page entry | `NS.Helpers` |
| `Tooltip.lua` | Windows-page entry: registers `tooltip` (30 rows across six tabs: General, Bar, Bar background, Bar border, Text, Contents). Pure schema | a Windows-page entry | `NS.Helpers` |
| `Visibility.lua` | Windows-page entry: registers `visibility` (17 rows across three tabs — where, extra rules, combat). Pure schema | a Windows-page entry | `NS.Helpers` |
...
3. **`core/`**, in this order and for these reasons:
   ...
   15. `Diagnostics_DeathRecap.lua`, 16. `Diagnostics_Identity.lua`, 17. `Diagnostics_Feign.lua`,
       18. `Diagnostics_Runtime.lua` —
       **all four after `Diagnostics.lua`, and load-bearingly so.** Each resolves `NS.Diagnostics`
...
5. **`modules/`** — `Format` first (nothing reads another module, and `Row` and `Tooltip` both format
   ...
   **`Export_Modal`** → `Visibility`.
   ...
   The eleven in bold are the layout-§1 peels, and **every one of their positions is load-bearing**
...
load: a private bus target carrying `RESTRICTION_CHANGED`, so a dialog left open across a pull grays
its own buttons.
