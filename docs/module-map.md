...
| `OptionsSetup.lua` | The options descriptor, the page registry, the reset-all veto (`page == "profiles"`), the one row no reset in the panel may write (`global.minimap.shown`, exempted at `applyDefault` — `launcher-§3`), the Windows page's entry registry and its band-rail-entry renderer (MultiMeters#55), and the library-absent stub | `NS.Helpers` (the library instance itself), `NS.CreateOptionsPanel`, `NS.OpenOptionsPanel`, `NS.RefreshOptionsPanel`, `NS.RegisterWindowSection`, `NS.WindowSection`, `Helpers.RenderWindowPage`, `Helpers.RestoreActiveSection` | LibKa0s-Options-1.0, `NS.Schema`, `NS.Slash:LandingRows` |
| `Windows.lua` | The window picker — `H.WindowBanner`, **the only writer of `NS.State.activeWindowId`**, decorated onto the library instance and drawn by all seven window pages — and the Windows page's General entry (MultiMeters#55): the five registry buttons plus the Copy settings from block with its group filter, on one bespoke General tab | a page registration and a Windows-page entry | `NS.WindowManager`, `NS.State.SetActiveWindow`, `NS.RefreshOptionsPanel` |
...
| `Header.lua` | The Header page (36 rows across four tabs): the title bar's own text, every toggle for the icon strip it carries (Controls) and how those controls are drawn (Button style). The column-header strip moved to the Columns page it labels. Rows are stored at `window.frame.*` / `window.header.*`. Pure schema | a page registration | `NS.Helpers` |
| `Bars.lua` | The Bars page (29 rows across six tabs: Bar, Background, Border, Text content, Text style, Icons): the bar, its background, its border, the cell's two text slots and the row icon. The Text and Icons pages folded in here; their PATHS did not move with them. Pure schema | a page registration | `NS.Helpers` |
