...
| under 1000 (left the band) | `tests/test_slash.lua` | 834 | **Peeled out of the band in MM-ATS-01 (the 2026-09-26 automated-tests sweep, ATS-12), along the seam this cell named.** 1411 → 834: the diagnostics verbs (`perf` and the whole `/mm debug` ladder, 20 cases) moved to `tests/test_slash_diagnostics.lua` (437 lines), and the disabled-state refusal cases (6) joined the schema-seam refusals in `tests/test_slash_refusal.lua` (141 → 325), because the diagnostics seam alone left the parent at 1010. 81 cases before and after (76 + 5 → 50 + 20 + 11). Drops off this list at the next run. |
