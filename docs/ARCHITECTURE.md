...
| `debug.md` | Not applicable | The console is `LibKa0s-DebugLog-1.0`'s window. `/mm debug` toggles it and takes `on` / `off`. This addon's own surface is `/mm debug tooltip`, `/mm debug diag`, `/mm debug recap`, `/mm debug identity` and `/mm debug feign` — the last armed and disarmed with `feign on` / `feign off`, which is why it is the one probe verb that reads an argument — `core/Diagnostics.lua` and the three probe siblings it was peeled into on 2026-09-09 for `layout-§1` (`Diagnostics_DeathRecap.lua`, `Diagnostics_Identity.lua`, `Diagnostics_Feign.lua`) — print statements whose headers explain themselves. The peel gave each long-lived probe the lifecycle it wanted: written for an issue, deletable with it. **The re-check trigger this row names has now FIRED**: `/mm debug tooltip` (2026-09-09) is a session flag on `NS.State` — state, and an option — added because the tooltip channel is rebuilt on every mouse-over and was evicting a capped buffer faster than a player could read it. It is one toggle that prints the state it landed in and is documented in the [Slash commands](#slash-commands) table and in the README, which is still less than a `debug.md` would carry; the next thing this surface gains, or a second flag, tips it and the doc should be written. It has grown — the death-recap probe for issue #1 is the newest section, the first with a verb of its own, and the first to search the client two ways because one was measured to be unreliable — so this is the Tier 2 trigger nearest to firing; re-measure it when a section gains state or an option. |
