...
| `resetall` | Reset the active profile to the shipped defaults — a **profile reset**, so it is the equivalent of a new profile: extra windows are deleted and one fresh window is left. The same act as Profiles → Reset Profile; other profiles are never touched. It calls `Helpers.RestoreAllDefaults`, the function the General page's popup calls. Until 2026-09-12 it went to the library's `CliResetAll`, which reset only the active window's rows. See [settings-panel.md](settings-panel.md#reset-all-settings-vs-reset-profile) |
