...
Fifty-nine non-vendored source files: 1 locale, 19 `core/`, 1 `defaults/`, 23 `modules/`, 15 `settings/`.
...
its seam falls. The two to watch are `modules/Window.lua` at 1490, which took the bound-slot render
(review F-007) and has ten lines of headroom, and `modules/Row.lua` at 1446 after its live-cell set and
